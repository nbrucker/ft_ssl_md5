...
void	ft_sha256_n1(t_sha256 *sha256, uint32_t j)
{
	while (j < 16)
	{
	...
	while (j < 64)
	{
		sha256->s0 = ft_right_rotate(sha256->chunk[j - 15], 7) ^
		ft_right_rotate(sha256->chunk[j - 15], 18) ^
		(sha256->chunk[j - 15] >> 3);
		sha256->s1 = ft_right_rotate(sha256->chunk[j - 2], 17) ^
		ft_right_rotate(sha256->chunk[j - 2], 19) ^
		(sha256->chunk[j - 2] >> 10);
		sha256->chunk[j] = sha256->chunk[j - 16] + sha256->s0 +
		sha256->chunk[j - 7] + sha256->s1;
		j++;
	}
}

void	ft_sha256_n2(t_sha256 *sha256, uint32_t j)
{
	while (j < 64)
	{
		sha256->s1 = ft_right_rotate(sha256->e, 6) ^ ft_right_rotate(
			sha256->e, 11) ^ ft_right_rotate(sha256->e, 25);
		sha256->ch = (sha256->e & sha256->f) ^ ((~sha256->e) & sha256->g);
		sha256->tmp1 = sha256->h + sha256->s1 + sha256->ch +
		g_k[j] + sha256->chunk[j];
		sha256->s0 = ft_right_rotate(sha256->a, 2) ^ ft_right_rotate(
			sha256->a, 13) ^ ft_right_rotate(sha256->a, 22);
...
void	ft_sha256_algo(t_sha256 *sha256, char *new, uint32_t i)
{
	ft_copy_len((void*)sha256->chunk, (void*)new + i, 64);
	ft_sha256_n1(sha256, 0);
	sha256->a = sha256->h0;
	sha256->b = sha256->h1;
	sha256->c = sha256->h2;
	sha256->d = sha256->h3;
...
void	ft_sha256(t_hash *hash)
{
	...
	{
		ft_sha256_algo(sha256, new, i);
		i += 64;
	}
	if (!(hash->hash = ft_strnew(65)))
		ft_error("Malloc error");
	ft_fill_hash_sha256(hash->hash, sha256->h0);
	ft_fill_hash_sha256(hash->hash + 8, sha256->h1);
	ft_fill_hash_sha256(hash->hash + 16, sha256->h2);
	ft_fill_hash_sha256(hash->hash + 24, sha256->h3);
	ft_fill_hash_sha256(hash->hash + 32, sha256->h4);
	ft_fill_hash_sha256(hash->hash + 40, sha256->h5);
	ft_fill_hash_sha256(hash->hash + 48, sha256->h6);
	ft_fill_hash_sha256(hash->hash + 56, sha256->h7);
	ft_memdel((void**)&new);
